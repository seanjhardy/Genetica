...
void DragHandler::render(VertexManager& vertexManager, const sf::FloatRect& bounds) {
    ...
    if (horizontalDirection() == 1) {
        vertexManager.addLine({bounds.left + bounds.width, bounds.top},
    ...
    if (verticalDirection() == 1) {
        vertexManager.addLine({bounds.left, bounds.top + bounds.height},
                              {bounds.left + bounds.width, bounds.top + bounds.height},
                              highlight, thickness);
    }
}
...
int DragHandler::horizontalDirection() const {
    ...
    else if (dragHandle == DragHandle::Right || dragHandle == DragHandle::TopRight || dragHandle ==
...
int DragHandler::verticalDirection() const {
    ...
    else if (dragHandle == DragHandle::Bottom || dragHandle == DragHandle::BottomLeft || dragHandle ==
        DragHandle::BottomRight) {
        return 1;
    }
    return 0;
