...
float dir(float2 p1, float2 p2) {
    float2 d = p1 - p2;
    return std::atan2f(d.s[1], d.s[0]);
}

float2 rotate(float2 point, float angle) {
    float2 d = vec(angle);
    float x = point.s[0] * d.s[0] - point.s[1] * d.s[1];
    float y = point.s[0] * d.s[1] + point.s[1] * d.s[0];
    return make_float2(x, y);
}

double2 rotate(double2 point, float angle) {
    float2 d = vec(angle);
    double x = point.s[0] * d.s[0] - point.s[1] * d.s[1];
    double y = point.s[0] * d.s[1] + point.s[1] * d.s[0];
...
float2 rotateOrigin(float2 point, float2 origin, float angle) {
    float2 d = vec(angle);
    float x = (point.s[0] - origin.s[0]) * d.s[0] - (point.s[1] - origin.s[1]) * d.s[1];
    float y = (point.s[0] - origin.s[0]) * d.s[1] + (point.s[1] - origin.s[1]) * d.s[0];
    return make_float2(x, y);
}

float diff(float2 p1, float2 p2) {
    return abs(p1.s[0] - p2.s[0]) + abs(p1.s[1] - p2.s[1]);
}

float sum(float2 p1) {
    return p1.s[0] + p1.s[1];
}
...
float angleBetween(float2 p1, float2 p2) {
    float2 d = p2 - p1;
    return atan2f(d.s[1], d.s[0]);
}

float magnitude(float2 p1) {
    return sqrtf(p1.s[0] * p1.s[0] + p1.s[1] * p1.s[1]);
}

double magnitude(double2 p1) {
    return sqrt(p1.s[0] * p1.s[0] + p1.s[1] * p1.s[1]);
}

float magnitude(float3 p1) {
    return sqrt(p1.s[0] * p1.s[0] + p1.s[1] * p1.s[1] + p1.s[2] * p1.s[2]);
}

std::vector<float2> findPerpendicularPoints(const Point& point1, const Point& point2, float r1, float r2) {
    float x1 = point1.pos.s[0], y1 = point1.pos.s[1];
    float x2 = point2.pos.s[0], y2 = point2.pos.s[1];
    ...
    double angle = atan2f(y2 - y1, x2 - x1);
