...
#include "../../modules/utils/random.cpp"
...
void Fish::render(VertexManager& viewer, bool stereoscopic, bool NPC) {
    if (!NPC) {
...
void Fish::drawVentralFins(VertexManager& viewer, float2 pos, float angle, float finSize, const sf::Color& finColour, float skew) {
    const float deg90 = M_PI / 2;
    skew = sin(skew);
    float skewLeft = skew > 0 ? 0 : std::abs(skew + 0.2f);
    float skewRight = skew < 0 ? 0 : std::abs(skew - 0.2f);

    std::vector<float2> leftFin;
    leftFin.push_back(pos + vec(angle + deg90) * finSize);
    leftFin.push_back(pos + vec(angle + deg90 * (2 + 0.5f * skewLeft)) * finSize * 3);
    leftFin.push_back(pos + vec(angle + deg90 * (1.5f + 0.3f * skewLeft)) * finSize * 5);
    viewer.addPolygon(leftFin, finColour);

    std::vector<float2> rightFin;
    rightFin.push_back(pos + vec(angle - deg90) * finSize);
    rightFin.push_back(pos + vec(angle - deg90 * (2 + 0.5f * skewRight)) * finSize * 3);
    rightFin.push_back(pos + vec(angle - deg90 * (1.5f + 0.3f * skewRight)) * finSize * 5);
    viewer.addPolygon(rightFin, finColour);
}
