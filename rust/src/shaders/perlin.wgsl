// Perlin noise shader for planet background generation
// Port of perlin.frag to WGSL
...
struct PlanetUniforms {
    ...
    noise_warp: f32,
    num_colors: u32,
    smooth_noise: u32,  // bool as u32
    _padding1: f32,
    _padding2: f32,
    offset: vec2<f32>,     // bounds offset (left, top)
    resolution: vec2<f32>, // bounds size (width, height)
}

@group(0) @binding(0)
var<uniform> uniforms: PlanetUniforms;
...
fn gradient_noise_2d(pos: vec2<f32>) -> vec2<f32> {
    let x = noise3D(pos.x, pos.y, 0.0);
    let y = noise3D(pos.x + 5.2, pos.y + 1.3, 0.0);
    return vec2<f32>(x, y) * 2.0 - 1.0;
}

fn warped_noise(pos: vec2<f32>, warp_amount: f32) -> f32 {
    let offset = gradient_noise_2d(pos) * warp_amount;
    return noise3D(pos.x + offset.x, pos.y + offset.y, 0.0);
}

fn fbm_warped(pos: vec2<f32>) -> f32 {
    var sum = 0.0;
    var amplitude = 1.0;
    var frequency = uniforms.noise_frequency;
    var warp_amount = uniforms.noise_warp;
    
    for (var i: u32 = 0u; i < uniforms.noise_octaves; i++) {
        sum += warped_noise(pos * frequency, warp_amount) * amplitude;
        amplitude *= 0.5;
        frequency *= 2.0;
        warp_amount *= 0.5;
    }
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let x = (in.uv.x * uniforms.resolution.x + uniforms.offset.x) / 500.0 + uniforms.seed * 1000.0;
    let y = (in.uv.y * uniforms.resolution.y + uniforms.offset.y) / 500.0;
    
    var noise_value = fbm_warped(vec2<f32>(x, y));
    
    // Normalize noise value to [0, 1] range
    noise_value = clamp((noise_value * 2.0 + 1.0) * 0.5, 0.0, 1.0);
    
    // Default to first color
    var color = colors[0];
    
    if uniforms.smooth_noise != 0u {
        // Smooth color interpolation
        let band_size = 1.0 / f32(uniforms.num_colors - 1u);
        
        for (var i: u32 = 0u; i < uniforms.num_colors - 1u; i++) {
            let lower_bound = band_size * f32(i);
            let upper_bound = band_size * f32(i + 1u);
            
            if noise_value >= lower_bound && (noise_value <= upper_bound || i == uniforms.num_colors - 2u) {
                let t = (noise_value - lower_bound) / band_size;
                color = mix(colors[i], colors[i + 1u], t);
                break;
            }
        }
    } else {
        // Discrete color bands
        let band_size = 1.0 / f32(uniforms.num_colors);
        
        for (var i: u32 = 1u; i < uniforms.num_colors; i++) {
            if noise_value > band_size * f32(i) {
                color = colors[i];
            }
        }
    }
